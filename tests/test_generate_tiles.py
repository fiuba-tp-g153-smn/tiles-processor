...
class TestGenerateTilesService:
    ...
    @pytest.mark.asyncio
    async def test_run_creates_output_directory(self, mock_geotiff_files, tmp_output_dir):
        """Test that run() creates the output directory."""
        service = GenerateTilesService(mock_geotiff_files, tmp_output_dir)

        with patch.object(service, '_generate_tiles_with_limit', new_callable=MagicMock) as mock_gen:
    ...
    async def test_run_processes_all_files(self, mock_geotiff_files, tmp_output_dir):
        ...
        with patch.object(service, '_generate_tiles_with_limit', side_effect=track_processing):
            await service.run()
    ...
    def test_generate_tiles_command_construction(self, mock_geotiff_files, tmp_output_dir):
    ...
    def test_generate_tiles_cleanup_on_failure(self, mock_geotiff_files, tmp_output_dir):
    ...
    def test_generate_tiles_overwrites_existing(self, mock_geotiff_files, tmp_output_dir):
