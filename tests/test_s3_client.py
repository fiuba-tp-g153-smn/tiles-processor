...
class TestS3ClientDownloadFile:
    ...
    async def test_download_file_retry_then_success(self):
        ...
        mock_s3_client.get_object = AsyncMock(side_effect=[
    ...
    async def test_download_file_respects_semaphore(self):
        ...
        tasks = [
            client.download_file(mock_s3_client, f"file{i}.nc")
            for i in range(5)
        ]
...
class TestS3ClientDownloadFolder:
    ...
    async def test_download_folder_with_filter(self):
        ...
        with patch.object(client, '_get_folder_file_paths', new_callable=AsyncMock) as mock_get_paths:
