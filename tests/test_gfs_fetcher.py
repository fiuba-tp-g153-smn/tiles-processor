...
from data_sources.gfs_fetcher import GfsGribFetcher, InvalidGribResponseError
...
from data_sources.gfs_repository import describe_step
from services.gfs_grib_validation import scan_grib, validate_gfs_grib
...
class TestValidation:
    def test_accepts_a_well_formed_subset(self, tmp_path: Path):
        ...
        validate_gfs_grib(target, PUBLIC_ENDPOINT, describe_step(CYCLE, 0))
    ...
    def test_error_message_names_the_endpoint(self, tmp_path: Path):
        ...
        with pytest.raises(InvalidGribResponseError, match=INTERNAL_ENDPOINT):
            validate_gfs_grib(target, INTERNAL_ENDPOINT, describe_step(CYCLE, 0))
...
class TestScanGrib:
    ...
    def test_a_whole_file_tiles_its_bytes_exactly(self, tmp_path: Path):
        ...
        assert scan_grib(target).ends_at_eof
    ...
    def test_returns_zero_for_a_non_grib_file(self, tmp_path: Path):
        ...
        assert scan_grib(target).message_count == 0
