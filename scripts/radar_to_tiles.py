...
def read_radar(filepath: str):
    ...
    print(f"  Centro: ({radar.latitude['data'][0]:.4f}, {radar.longitude['data'][0]:.4f})")
    return radar
...
def radar_to_grid(radar, sweep: int = 0, resolution: int = 500):
    ...
    grid = pyart.map.grid_from_radars(
        (radar,),
        grid_shape=(1, int(2*max_range/resolution), int(2*max_range/resolution)),
        grid_limits=(
            (0, 10000),
            (-max_range, max_range),
            (-max_range, max_range),
...
def grid_to_rgba_geotiff(grid, output_path: Path, field_name: str, variable: str):
    ...
    with rasterio.open(
        ...
        height=nrows, width=ncols, count=4,
        dtype=np.uint8, crs=CRS.from_epsg(4326),
        transform=transform, compress='lzw',
    ) as dst:
        for i in range(4):
            dst.write(np.flipud(rgba_uint8[:, :, i]), i + 1)
    
    print(f"    Bounds: ({min_lon:.2f}, {min_lat:.2f}) - ({max_lon:.2f}, {max_lat:.2f})")
...
def geotiff_to_tiles(geotiff_path: Path, tiles_dir: Path, zoom_levels: str = "4-10"):
    ...
    cmd = [
        "gdal2tiles.py", "-p", "mercator", "-z", zoom_levels,
        "-w", "none", f"--processes={num_processes}",
        "--tiledriver=WEBP",
        str(geotiff_path), str(tiles_dir)
    ]
...
def process_all_radar_files(pattern: str = "RMA*.H5", output_dir: Path = OUTPUT_DIR, max_workers: int = None):
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        futures = {executor.submit(process_radar_file, filepath, output_dir, [0, 1, 2]): filepath for filepath in files}
